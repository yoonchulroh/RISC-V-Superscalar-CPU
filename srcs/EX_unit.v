...
module EX_unit (
    ...
    // Signals for MUL unit
    wire MUL_unit_multiply;
    wire [2:0] MUL_unit_funct3;
    wire [31:0] MUL_unit_inputx, MUL_unit_inputy;
    wire MUL_unit_multiplying, MUL_unit_result_ready;
    wire [31:0] MUL_unit_result;

    // EXBR - saved infos
    reg [31:0] saved_EXBR_instruction, saved_EXBR_pcplusfour;
    reg [31:0] saved_EXBR_pc;
    reg [31:0] saved_EXBR_speculated_next_pc;
    ...
    MUL_unit MUL_unit_inst (
        ...
        .reset(reset),
        .CPU_state(CPU_state),
        .hazard_EXBR_bubble(hazard_EXBR_bubble),

        .multiply(MUL_unit_multiply),
        .funct3(MUL_unit_funct3),
        .inputx(MUL_unit_inputx),
        .inputy(MUL_unit_inputy),

        .multiplying(MUL_unit_multiplying),
        .result_ready(MUL_unit_result_ready),
        .result(MUL_unit_result)
    );

    always @(posedge clk) begin
        if (reset || CPU_state == 0 || hazard_EXBR_bubble || MUL_unit_multiplying) begin
            EXBR_branch <= 0;
            EXBR_jump <= 0;
            EXBR_memread <= 0;
            EXBR_memwrite <= 0;
        ...
        else if (MUL_unit_result_ready) begin
            EXBR_branch <= 0;
            EXBR_jump <= 0;
            EXBR_memread <= 0;
            EXBR_memwrite <= 0;
            ...
            EXBR_stop <= 0;
            EXBR_print <= 0;
            EXBR_flush_upon_wrong_speculation <= 1;
            EXBR_reserve <= 0;
            EXBR_trap <= 0;
            EXBR_CSR_file_write <= 0;
            EXBR_write_mcause <= 0;

            EXBR_pcplusfour <= saved_EXBR_pcplusfour;
            EXBR_instruction <= saved_EXBR_instruction;
            EXBR_ALU_result <= MUL_unit_result;
            EXBR_toreg <= 0;
            EXBR_pc <= saved_EXBR_pc;
            EXBR_branch_control_taken <= 0;
            EXBR_speculated_next_pc <= saved_EXBR_speculated_next_pc;
            EXBR_tentative_register_file_data_to_write <= MUL_unit_result;
        end
        else begin
        ...
        if (IDEX_multiply) begin
            saved_EXBR_pcplusfour <= IDEX_pc + 4;
            saved_EXBR_instruction <= IDEX_instruction;
            saved_EXBR_pc <= IDEX_pc;
            saved_EXBR_speculated_next_pc <= IDEX_speculated_next_pc; 
        end
    ...
    assign MUL_unit_multiply = IDEX_multiply && ~reset && (CPU_state == 1) && ~hazard_EXBR_bubble;
    assign MUL_unit_funct3 = IDEX_instruction[14:12];
    assign MUL_unit_inputx = new_readdata1;
    assign MUL_unit_inputy = new_readdata2;

    assign hazard_IDEX_memread = IDEX_memread;
    assign hazard_IDEX_rd = IDEX_instruction[11:7];
    assign hazard_EX_multiplying = MUL_unit_multiplying;
